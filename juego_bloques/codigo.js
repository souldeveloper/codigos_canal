...
//Definir todos los bloques que
/* He añadido estas líneas para generar más bloques y de manera automatica sin hardcodear las coordeandas x / y */
let bloques = [];
const columnas = 5;
const cantidasDeBloques = 15;
const bloquesPorColumna = cantidasDeBloques / columnas;
const XLimit = 450;
let YLimit = 250;
const XSumar = 110;
const YRestar = 30;

for (let index = 0; index < bloquesPorColumna; index++) {
  for (let i = 10; i <= XLimit; i += XSumar) {
    const nuevoBloque = new Bloque(i, YLimit);
    bloques.push(nuevoBloque);
  }
  YLimit = YLimit - YRestar;
}
